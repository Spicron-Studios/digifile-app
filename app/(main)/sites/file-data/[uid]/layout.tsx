...
export default function FileDataLayout({
  ...
  const isDetailPage =
    pathname.includes('/file-data/') && params?.uid !== 'new-record';
  const isNewRecord = params?.uid === 'new-record';

  // Set up event listener for header data
  useEffect(() => {
    // Function to handle the custom event
    const handleHeaderData = (event: Event) => {
      const customEvent = event as CustomEvent<HeaderData>;
      setHeaderData(customEvent.detail);
    };
      ...
      {(isDetailPage || isNewRecord) && (
        <div className="p-4 border-b flex justify-between items-center bg-white">
          <div className="flex items-center">
            <Link
              href="/sites/file-data"
              className="flex items-center mr-6 text-gray-600 hover:text-gray-900"
            >
